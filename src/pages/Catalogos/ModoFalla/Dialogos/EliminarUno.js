...
const EliminarUno = ({ deleteProductDialog, hideDeleteProductDialog, product, _deleteProduct }) => {
    ...
    //--------------------| Valor que regresara |--------------------
    return (
        <Dialog visible={deleteProductDialog} style={{ width: "450px" }} header="Confirm" modal footer={unRegistro} onHide={hideDeleteProductDialog}>
            <div className="confirmation-content">
                <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: "2rem" }} />
                {/* CAMBIAR... */}
                {product && (
                    <span>
                        Estas seguro de eliminar <b>{product.nombreModoFalla}</b>?
                    </span>
                )}
            </div>
        </Dialog>
